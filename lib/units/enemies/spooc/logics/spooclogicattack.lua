...
function SpoocLogicAttack._upd_spooc_attack(data, my_data)
	...
	if not record.is_deployable and not record.status and not my_data.spooc_attack and focus_enemy.verified then
		if focus_enemy.verified_dis < (my_data.attitude == "engage" and 1500 or 900) and not data.unit:movement():chk_action_forbidden("walk") and (not my_data.last_dmg_t or data.t - my_data.last_dmg_t > 0.6) then
