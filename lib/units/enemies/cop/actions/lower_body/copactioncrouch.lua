...
function CopActionCrouch:init(action_desc, common_data)
	...
	if ext_anim.move then
		...
		if ext_anim.run_start_turn then
			...
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch.run_start_turn[move_side]
		elseif ext_anim.run_start then
			local move_side = ext_anim.move_side
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch.run_start[move_side]
		elseif ext_anim.run_stop then
			local move_side = ext_anim.move_side
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch.run_stop[move_side]
		else
			local pose = "crouch"
			...
			local walk_pose_tbl = walk_anim_lengths and walk_anim_lengths[pose]
