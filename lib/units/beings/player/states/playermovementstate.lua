...
function PlayerMovementState:enter(enter_data)
end
function PlayerMovementState:exit()
end
...
function PlayerMovementState:chk_action_forbidden(action_type)
	if self._current_action then
		...
		if unblock_data and (unblock_data == -1 or unblock_data > TimerManager:game():time()) then
			return true
		end
	end
end
