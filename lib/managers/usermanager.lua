...
function GenericUserManager:setup_setting_map()
	...
	self:setup_setting(10, "hold_to_run", not is_ps3 and true)
	...
	self:setup_setting(25, "fov_zoom", 60)
...
function GenericUserManager:check_user_state_change(old_user_data, user_data, ignore_username_change)
	...
	if old_signin_state ~= signin_state or not ignore_username_change and old_username ~= username then
	...
	if not ignore_storage_change and (active_user_changed or storage_id ~= old_storage_id) then
		self:storage_changed(user_data)
...
function GenericUserManager:storage_changed(user_data)
	managers.savefile:storage_changed()
	self._storage_changed_callback_handler:dispatch(user_data)
end
...
WinUserManager = PS3UserManager or class(GenericUserManager)
