...
function CoreSoundEnvironmentManager:add_check_object(data)
	...
	local t = {
		...
		surround = surround,
		surround_iterator = surround and 1 or nil,
		active = data.active,
...
function CoreSoundEnvironmentManager:_update_object(t, dt, id, data)
	...
	if data.surround then
		...
		data.surround_iterator = math.mod(data.surround_iterator + 1, 4)
	end
	if t > data.next_occasional then
		...
		if self._ambience_enabled then
			...
			if event then
				...
				local pos = check_pos + Vector3(x, y, 0):normalized() * 15000
				local sound_source = SoundDevice:create_source("occasional")
