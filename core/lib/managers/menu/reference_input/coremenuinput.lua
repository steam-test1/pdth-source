...
function MenuInput:back(queue)
	self._logic:navigate_back(queue == true or false)
end
...
function MenuInput:any_keyboard_used()
	if self._keyboard_used or not self._controller then
		return
	end
	for _, key in ipairs({
		"menu_right",
