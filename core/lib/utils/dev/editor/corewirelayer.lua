...
function WireLayer:update_unit_settings()
	...
	if self._selected_unit then
		self._ctrlrs.wire_slack:set_value(self._selected_unit:wire_data().slack)
	else
		self._ctrlrs.wire_slack:set_value(0)
	end
end
...
function WireLayer:build_panel(notebook)
	...
	slack_sizer:add(EWS:StaticText(self._ews_panel, "Wire Slack", 0, ""), 0, 0, "ALIGN_CENTER_HORIZONTAL")
	local wire_slack = EWS:Slider(self._ews_panel, 0, 0, 5000, "", "")
	slack_sizer:add(wire_slack, 0, 0, "EXPAND")
	wire_slack:connect("EVT_SCROLL_CHANGED", callback(self, self, "change_slack"), wire_slack)
	wire_slack:connect("EVT_SCROLL_THUMBTRACK", callback(self, self, "change_slack"), wire_slack)
	self._sizer:add(slack_sizer, 1, 0, "EXPAND")
	self._main_sizer:add(self._sizer, 1, 0, "EXPAND")
	self._ctrlrs.wire_slack = wire_slack
